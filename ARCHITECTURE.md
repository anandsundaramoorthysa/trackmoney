...
```
   ...
   an AI buyer ────────▶│  billing · insights · trail   │
      ...
      │ 14 tables     │        │ → templates     │        │ Orders API     │
...
```
  ...
  ② classifyIntent()            ◀── NO MODEL. Plain code.
     yes / no / question / greeting / unclear     lib/agent/intent.ts
          │
          ▼
...
|---|---|---|
| Only three tools exist | a name check in `run.ts` | the prompt |
...
| The three tools and their gates | `lib/agent/tools.ts` |
